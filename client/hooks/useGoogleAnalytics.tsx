...
export const useGoogleAnalytics = () => {
  ...
  // Track video view
  const trackVideoView = (videoId: string, videoTitle: string, userId?: string) => {
    if (typeof window.gtag !== 'undefined') {
  ...
  const trackCreatorContent = (action: string, contentId: string, creatorId?: string) => {
    if (typeof window.gtag !== 'undefined') {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const generatedRevenueData = months.map((month, index) => ({
        month,
        revenue: Math.floor((realData.revenue / 6) * (1 + index * 0.2) + (Math.random() * 200)),
        views: Math.floor((realData.views / 6) * (1 + index * 0.15) + (Math.random() * 1000))
      }));
      ...
      const generatedVideosData = videoTitles.slice(0, Math.min(realData.videos, 5)).map((title, index) => ({
        id: index + 1,
        title,
