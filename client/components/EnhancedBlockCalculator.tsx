...
export const EnhancedBlockCalculator: React.FC<EnhancedBlockCalculatorProps> = ({
  onCalculationChange,
  onPaymentRequired,
  className = ''
}) => {
  ...
  const calculateBlocks = () => {
    if (!videoSpecs.duration || !videoSpecs.resolution || !videoSpecs.quantity) {
      setCalculation(null);
    ...
    const result: CalculationResult = {
      ...
      canAfford: creatorBlocks ? blocksNeeded <= creatorBlocks.availableBlocks : false,
  ...
  const handleInputChange = (field: keyof VideoSpecs, value: string | number) => {
    setVideoSpecs(prev => ({
      ...prev,
      [field]: typeof value === 'string' ? (field === 'resolution' ? value : Number(value)) : value
    }));
  };
  ...
  const handleBuyBlocks = () => {
    if (calculation && onPaymentRequired) {
      const blocksNeeded = Math.max(1, calculation.blocksNeeded - calculation.availableBlocks);
            ...
            <div className="bg-muted/50 rounded-lg p-4">
              ...
              <div className="grid md:grid-cols-3 gap-4 text-sm">
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-600">{creatorBlocks.availableBlocks}</p>
                  <p className="text-muted-foreground">Disponíveis</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-purple-600">{creatorBlocks.usedBlocks}</p>
                  <p className="text-muted-foreground">Em Uso</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-green-600">{creatorBlocks.totalBlocks}</p>
              ...
              {creatorBlocks.isGracePeriod && (
                <Alert className="mt-4 border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950">
                  ...
                  <AlertDescription className="text-green-800 dark:text-green-200">
                    <strong>Período de Carência Ativo!</strong> Você tem {creatorBlocks.freeMonthsLeft} mês{creatorBlocks.freeMonthsLeft > 1 ? 'es' : ''} gratuitos restantes.
                  </AlertDescription>
                </Alert>
              )}
            </div>
              ...
              <div className="grid md:grid-cols-4 gap-4">
                ...
                <div className="bg-green-500/10 rounded-lg p-4 text-center">
                  ...
                  <p className="font-bold text-lg">
                    {calculation.breakdown.freeMonthsApplied > 0 ? (
                      ...
                      <span className="text-green-600">{formatPrice(calculation.totalPrice)}</span>
                    )}
                  </p>
                </div>
                ...
                <div className="bg-xnema-orange/10 rounded-lg p-4 text-center">
                  ...
                  <Badge className={calculation.canAfford ? 'bg-green-500' : 'bg-red-500'}>
                    {calculation.canAfford ? 'Aprovado' : 'Blocos Insuficientes'}
                  </Badge>
                </div>
              </div>
              ...
              <Alert className="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950">
                ...
                <AlertDescription className="text-blue-800 dark:text-blue-200">
                  <div className="space-y-1 text-sm">
                    ...
                    <p>• {videoSpecs.quantity} vídeo(s) em {VIDEO_SPECS[videoSpecs.resolution].quality}</p>
                    <p>• {videoSpecs.duration} min cada = {calculation.breakdown.sizePerVideo} GB por vídeo</p>
                    <p>• {calculation.breakdown.blocksPerVideo} bloco(s) por vídeo</p>
                    <p>• Total: {calculation.totalSizeGB} GB = {calculation.blocksNeeded} bloco(s)</p>
                    {calculation.breakdown.freeMonthsApplied > 0 && (
                      <p className="text-green-600">• <strong>Período gratuito aplicado!</strong></p>
                    )}
                  </div>
                </AlertDescription>
              </Alert>
                ...
                <Alert className="border-red-500 bg-red-50 dark:border-red-800 dark:bg-red-950">
                  ...
                  <AlertDescription className="text-red-800 dark:text-red-200">
                    <div className="space-y-3">
                      <p><strong>Blocos Insuficientes:</strong> Você precisa de {calculation.blocksNeeded} blocos, mas tem apenas {calculation.availableBlocks} disponíveis.</p>
                      ...
                      >
                        ...
                        Comprar {Math.max(1, calculation.blocksNeeded - calculation.availableBlocks)} Bloco(s)
                        <span className="ml-2">
                          {formatPrice((calculation.blocksNeeded - calculation.availableBlocks) * BLOCK_PRICE)}
                        </span>
                      </Button>
                    </div>
                  </AlertDescription>
          ...
          {recommendation.recommended !== videoSpecs.resolution && (
            <Alert className="border-yellow-500 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-950">
              ...
              <AlertDescription className="text-yellow-800 dark:text-yellow-200">
                <p><strong>Sugestão:</strong> Para vídeos de {videoSpecs.duration} minutos, recomendamos <strong>{VIDEO_SPECS[recommendation.recommended as keyof typeof VIDEO_SPECS].quality}</strong>.</p>
                <p className="text-sm mt-1">{recommendation.reason}</p>
                <Button 
                  variant="outline" 
                  size="sm" 
                  className="mt-2"
                  onClick={() => handleInputChange('resolution', recommendation.recommended)}
                >
        ...
        <CardContent>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  ...
                  const sizes = {
                    '720p': Math.round(duration * VIDEO_SPECS['720p'].sizePerMinute * 100) / 100,
                    '1080p': Math.round(duration * VIDEO_SPECS['1080p'].sizePerMinute * 100) / 100,
                    '4k': Math.round(duration * VIDEO_SPECS['4k'].sizePerMinute * 100) / 100
                  };
                  ...
                  return (
                    <tr key={duration} className="border-b">
                      ...
                      <td className="p-2">
                        {sizes['720p']} GB • {blocks['720p']} bloco{blocks['720p'] > 1 ? 's' : ''}
                      </td>
                      <td className="p-2">
                        {sizes['1080p']} GB • {blocks['1080p']} bloco{blocks['1080p'] > 1 ? 's' : ''}
                      </td>
                      <td className="p-2 text-yellow-600">
                        {sizes['4k']} GB • {blocks['4k']} bloco{blocks['4k'] > 1 ? 's' : ''}
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="mt-4 p-3 bg-muted/50 rounded-lg">
            <p className="text-sm text-muted-foreground">
              <strong>Fórmula:</strong> 1 bloco = 7,3 GB = R$ 1.000 • 
              Cálculos baseados em bitrates profissionais • 
              Primeiro mês gratuito para novos criadores
            </p>
          </div>
        </CardContent>
      </Card>
