...
class SupabaseAuthService {
   ...
   */
  async createUser(userData: Omit<SupabaseUser, 'id' | 'created_at' | 'updated_at'>): Promise<{ user: SupabaseUser | null; error: string | null }> {
    try {
   ...
   */
  async getUserByEmail(email: string): Promise<{ user: SupabaseUser | null; error: string | null }> {
    try {
      const { data, error } = await supabase
        .from('users')
        .select('*')
   ...
   * Atualizar usuário
   */
  async updateUser(userId: string, updates: Partial<SupabaseUser>): Promise<{ user: SupabaseUser | null; error: string | null }> {
    try {
      const { data, error } = await supabase
        .from('users')
        .update({
   ...
   * Ativar assinatura
   */
  async activateSubscription(userId: string, plan: string, paymentId?: string): Promise<{ success: boolean; error: string | null }> {
    try {
   ...
   * Registrar pagamento
   */
  async recordPayment(paymentData: Omit<PaymentRecord, 'id' | 'created_at'>): Promise<{ payment: PaymentRecord | null; error: string | null }> {
    try {
      const { data, error } = await supabase
        .from('payments')
        .insert([{
   ...
   */
  async getContent(filters?: { type?: string; genre?: string; isPremium?: boolean }): Promise<{ content: ContentItem[]; error: string | null }> {
    try {
   ...
   */
  async getRecommendations(userId: string): Promise<{ content: ContentItem[]; error: string | null }> {
    try {
   ...
   */
  async getPopularContent(): Promise<{ content: ContentItem[]; error: string | null }> {
    try {
      const { data, error } = await supabase
        .from('content')
        .select('*')
   ...
   */
  async recordView(userId: string, contentId: string): Promise<{ success: boolean; error: string | null }> {
    try {
