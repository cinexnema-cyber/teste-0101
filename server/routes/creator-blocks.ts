 ...
 */
export const getCreatorBlocks = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const creatorBlocks = await CreatorBlocks.getOrCreateForCreator(creatorId, user.name);
 ...
 */
export const checkUploadCapacity = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { creatorId } = req.params;
    const { error, value } = calculateBlocksSchema.validate(req.body);
    ...
    res.json({
      ...
      suggestedAction: uploadCheck.canUpload ? null : {
        type: 'purchase',
        blocksNeeded: uploadCheck.blocksNeeded,
        minBlocksToPurchase: Math.max(1, (uploadCheck.blocksNeeded || 1) - creatorBlocks.availableBlocks)
      }
    });
 ...
 */
export const purchaseBlocks = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { creatorId } = req.params;
...
export const handleBlocksWebhook = async (req: Request, res: Response) => {
  try {
    ...
    if (action === "payment.created" || action === "payment.updated") {
      ...
      const creatorBlocks = await CreatorBlocks.getByTransactionId(externalReference);

      if (!creatorBlocks) {
        console.error("Compra de blocos não encontrada para transaction_id:", externalReference);
      ...
      if (paymentStatus === "approved") {
        ...
        console.log(`Compra de blocos aprovada para criador ${creatorBlocks.creatorName} - Transaction: ${externalReference}`);

      } else if (paymentStatus === "rejected") {
        // Reject purchase
        await creatorBlocks.rejectPurchase(externalReference);

        console.log(`Compra de blocos rejeitada para transaction_id: ${externalReference}`);
      }
    }
 ...
 */
export const getPurchaseHistory = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { creatorId } = req.params;
 ...
 */
export const addVideoToBlocks = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { creatorId } = req.params;
    ...
    if (error instanceof Error && error.message === 'Blocos insuficientes para este vídeo') {
      return res.status(400).json({ 
 ...
 */
export const removeVideoFromBlocks = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { creatorId } = req.params;
    ...
    if (!creatorBlocks) {
      return res.status(404).json({ message: "Blocos do criador não encontrados" });
    }
    ...
    if (error instanceof Error && error.message === 'Video not found') {
      return res.status(404).json({ message: "Vídeo não encontrado nos blocos" });
    }
 ...
 */
export const getAllCreatorsBlocks = async (req: AuthenticatedRequest, res: Response) => {
  try {
    // Verify admin access
    ...
    const totals = {
      ...
      totalRevenue: creatorsBlocks.reduce((sum, cb) => sum + cb.stats.totalRevenue, 0),
      totalVideos: creatorsBlocks.reduce((sum, cb) => sum + cb.stats.totalVideoCount, 0)
    };
