...
export const MuxHelpers = {
   ...
   */
  createAsset: async (videoUrl: string, metadata: {
    title: string;
    description: string;
    creatorId: string;
  }) => {
  ...
  getThumbnailUrl: (playbackId: string, options?: {
    ...
    const { width = 640, height = 360, format = 'jpg', time = 0 } = options || {};
  ...
  validateVideoFile: (file: Express.Multer.File) => {
    ...
    if (!validTypes.includes(file.mimetype)) {
      return {
        valid: false,
        error: 'Tipo de arquivo inválido. Apenas MP4, MOV, AVI e WebM são permitidos.'
      };
    }
